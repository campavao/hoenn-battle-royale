...
const state = () => page.evaluate(() => {
  ...
  return {
    ...
    gl: gl ? { viewport: [...gl.getParameter(gl.VIEWPORT)], buffer: [gl.drawingBufferWidth, gl.drawingBufferHeight] } : null,
...
for (let t = 0; t < seconds; t++) {
  ...
  const s = await state();
  console.log(`${String(t).padStart(3)}s canvas ${s.canvas.w}x${s.canvas.h} css ${s.canvas.css.join('x')} top ${s.canvas.top} gl ${s.gl ? s.gl.viewport.join(',') + ' buf ' + s.gl.buffer.join('x') : '-'} clip ${s.clip} box ${s.box.join('x')}`);
