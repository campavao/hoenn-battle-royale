...
| `bt` | `BR_MSG_BT` 6 | ROM&lt;-&gt;page&lt;-&gt;page&lt;-&gt;ROM | yes | one raw link-block exchange, per turn, during a PvP battle. A bag item's return value (Emerald's ONERETURNVALUE) says whom it went to in its last byte, which pret leaves 0 -- the party slot + 1 and the move (`BR_ITEM_TARGET`) -- and the other ROM uses the item on its own copy of that mon as the block arrives (POK-331 #7) |
