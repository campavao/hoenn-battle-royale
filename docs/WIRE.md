...
| `challenge` | `BR_MSG_CHALLENGE` 5 | page&lt;-&gt;page (negotiate), page-&gt;ROM (engage) | yes (once resolved) | challenger; then both sides' pages push the resolved engage to their own ROM. The challenged ROM starts a fight only in the match proper (`BR_PHASE_PLAY`) and only from a seat whose ghost stands on its map, its own eyeline's rule (POK-330 #24). One that lands in a menu or under the fade into another screen waits for it to settle; a battle drops it, and a staged bot's is always a trainer battle, never a link (POK-330 #45) |
| `accept` | -- | page&lt;-&gt;page | no | the challenged seat, accepting |
| `decline` | -- | page&lt;-&gt;page | no | the challenged seat, declining |
| `bt` | `BR_MSG_BT` 6 | ROM&lt;-&gt;page&lt;-&gt;page&lt;-&gt;ROM | yes | one raw link-block exchange, per turn, during a PvP battle. Sent `to` the one seat being fought, never broadcast. A JSON-only `fight` names the challenge that started the fight (challenger's seat * 0x10000 + its nonce & 0xffff, `bridge.ts`'s `fightOf`), and a page refuses a block naming another fight than its own -- a rejoin says its last few blocks again, and those can be the last fight's (POK-331 #3). A block with no `fight` is still taken. A bag item's return value (Emerald's ONERETURNVALUE) says whom it went to in its last byte, which pret leaves 0 -- the party slot + 1 and the move (`BR_ITEM_TARGET`) -- and the other ROM uses the item on its own copy of that mon as the block arrives (POK-331 #7) |
...
| `bstart` | `BR_MSG_BSTART` 18 | ROM-&gt;page-&gt;page-&gt;ROM | yes | the challenger's ROM, when a link battle begins; carries seed + both parties + names so a spectator replays it as a BATTLE_TYPE_RECORDED. Also the host's hidden proxy instance, for the bot-vs-bot duel it is fighting (POK-300): published under the two bots' seats with a link battle's flags, relayed to the room by the host's page, which writes the bots' names into bytes 8..23 on the way (the instance has no roster) |
| `turn` | `BR_MSG_TURN` 19 | ROM-&gt;page-&gt;page-&gt;ROM | yes | the challenger's ROM, streaming the battle's new action bytes each turn so the spectator's replay stays a turn behind. A turn goes out once every battler has chosen; a RUN carries its kind and a bag item its id and target, so the replay plays a DOLL and a POTION (POK-330 #12) -- the other trainer's too, marked `BR_ITEM_PEER` so the replay does not read it as an AI's item type (POK-331 #7) |
...
| `give` | `BR_MSG_GIVE` 30 | page-&gt;ROM | yes | a player's own page, when its seat takes a bag off the ground (POK-280): the stacks inside it, which only the page kept. Never on the relay. A stack the bag has no room for goes back on the ground as a `spill` from that ROM: the same bag key and cell, under the taker's seat, with only the rest in it and no money (POK-331 #6) |
