...
| `turn` | `BR_MSG_TURN` 19 | ROM-&gt;page-&gt;page-&gt;ROM | yes | the challenger's ROM, streaming the battle's new action bytes each turn so the spectator's replay stays a turn behind. A turn goes out once every battler has chosen; a RUN carries its kind and a bag item its id and target, so the replay plays a DOLL and a POTION (POK-330 #12) |
