...
-- one `[mm:ss]` line per remark with a frame grab of the screen at that moment (the
frames are regenerated from the video and not committed). Whisper runs on the GPU
locally; 7.5 minutes took 37 seconds. A remark's frame is what to look at first; for a
moment that needs more, cut a contact sheet: `ffmpeg -ss <t> -t 10 -i <video> -vf
"crop=560:1500:0:380,fps=1,scale=300:-2,tile=5x2" -frames:v 1 sheet.jpg`.

**Fixed in the same commit as this entry:**

* The map past the picture flashed white around a Safari catch and black after a battle
  while the picture itself had not (01:05, "weird graphical issues at the top ... trying
  to catch up"). The catch's flash is a palette fade of the OBJ palettes only;
  `field.ts` now follows a fade only when it touches the BG palettes, and the black hold
  across a map load is capped at a second.
* The band the core draws past the LCD stayed on for the drop map, which runs under the
  overworld's callback: garbage tile rows above and below the region map (02:50). The
  band is clipped while `gBrPick.active`.
* The trainer Cam had just beaten kept flashing above the map (05:35): the overlay drew
  every sprite the ROM had marked invisible, and a despawning trainer is one. It draws
  only what the ROM's own `offScreen` bit hid.
